...
vegeta attack -rate=1/s -lazy -format=json -duration=5m < <(
    while true; do
        export BODY=$(echo "-Ls https://picsum.photos/1000 --output -" | curl -s -XPOST --data-binary @- http://$HOST/function/curl --output -)
        jq -ncM --arg HOST "$HOST" --arg BODY "$BODY" '{method: "POST", url: "http://\(env.HOST)/function/shasum", body: env.BODY | @base64, header: {"Content-Type": ["text/plain"]}}'
    done
) | \
    tee latencies.bin | \
